


## JUNIOR LARAVEL APP V1

Project Description: 

A simple project to test Junior Laravel Developer skilss.

Server Requirements: 

the same as for every Laravel 5.* project: PHP >= 7.1.3, OpenSSL PHP Extension, PDO PHP Extension, Mbstring PHP Extension, Tokenizer PHP Extension, XML PHP Extension, Ctype PHP Extension, JSON PHP Extension, BCMath PHP Extension.

Project start:

- composer install

- change DB name and password in .env file

- configure EMAIL credentials in .env file

- php artisan key:generate

- php artisan storage:link

- APP_URL=http://your-app-name.test

- FILESYSTEM_DRIVER=public

- php artisan migrate


- php artisan db:seed

- Login with default User at /login

- Default User Credentials: admin@admin.com, password

